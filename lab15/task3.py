...
def binary_insertion_sort(filename):
    ...
    record_size = struct.calcsize('i')  # Размер одного элемента в байтах

    with open(filename, 'r+b') as file:
        # Получаем количество элементов в файле
        file.seek(0, os.SEEK_END)
        file_size = file.tell()
        num_elements = file_size // record_size

        for i in range(1, num_elements):
            current_value = read_element(file, i, record_size)
            # Находим позицию для вставки текущего элемента
            insert_pos = binary_search(file, current_value, 0, i, record_size)

            # Сдвигаем элементы вправо для освобождения места
            for j in range(i, insert_pos, -1):
                prev_value = read_element(file, j - 1, record_size)
                write_element(file, j, prev_value, record_size)

            # Вставляем текущий элемент на правильную позицию
            write_element(file, insert_pos, current_value, record_size)